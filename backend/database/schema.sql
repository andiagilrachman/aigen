-- =============================================================================
--  AIGen — Skema Database  (aigen_db)
-- =============================================================================
--  Dihasilkan dari dump phpMyAdmin, DITAMBAH perbaikan hasil audit:
--
--   1. credit_wallets.balance  -> UNSIGNED + CHECK (balance >= 0)
--      Saldo kredit tidak akan pernah bisa minus, dijamin oleh database.
--   2. FK keuangan (credit_transactions, payments) CASCADE -> RESTRICT
--      Menghapus user tidak lagi menghapus jejak keuangan.
--   3. sectors.sub_sector NOT NULL DEFAULT ''
--      NULL tidak menahan duplikat pada UNIQUE KEY di MariaDB.
--   4. DROP TABLE IF EXISTS + CREATE DATABASE, supaya bisa di-import ulang.
--   5. Urutan tabel disusun agar FK selalu valid saat import berurutan.
--   6. coming_soon_items: UNIQUE (nav_menu_id) — agar seed idempotent.
--   7. credit_packages: UNIQUE (name) — agar seed idempotent.
--
--  Import:  mysql -u root -p < database/schema.sql
--  Lalu  :  mysql -u root -p aigen_db < database/seed.sql
-- =============================================================================
...
CREATE DATABASE IF NOT EXISTS `aigen_db`
  DEFAULT CHARACTER SET utf8mb4
  COLLATE utf8mb4_unicode_ci;
USE `aigen_db`;

-- Bersihkan tabel lama (urutan terbalik agar FK aman)
DROP TABLE IF EXISTS `watchlists`, `vendor_usage_log`, `user_subscriptions`, `user_sessions`, `users`, `trial_usage`, `theme_presets`, `system_settings`, `subscription_tiers`, `stock_management`, `stocks`, `shareholder_composition`, `sectors`, `price_seasonality`, `payments`, `nav_menu`, `indicator_snapshot_fundamental`, `indicator_history_fundamental`, `formula_config`, `feature_flags`, `data_vendors`, `credit_wallets`, `credit_transactions`, `credit_packages`, `credit_costs`, `corporate_actions`, `coming_soon_votes`, `coming_soon_subscriptions`, `coming_soon_items`, `api_response_cache`, `activity_logs`;
...
ALTER TABLE `credit_transactions`
  ADD CONSTRAINT `fk_credittx_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT;
ALTER TABLE `credit_transactions`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;
...
CREATE TABLE `credit_wallets` (
  `id` bigint(20) UNSIGNED NOT NULL,
  `user_id` bigint(20) UNSIGNED NOT NULL,
  `balance` int(10) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'UNSIGNED: saldo tidak bisa minus',
  `updated_at` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
ALTER TABLE `payments`
  ADD CONSTRAINT `fk_payments_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE RESTRICT;
ALTER TABLE `payments`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT;
...
CREATE TABLE `sectors` (
  ...
  `sub_sector` varchar(100) NOT NULL DEFAULT '' COMMENT 'NOT NULL agar UNIQUE KEY menahan duplikat',
  `created_at` datetime NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
ALTER TABLE `sectors`
  ADD PRIMARY KEY (`id`),
...
-- Fix 6: coming_soon_items butuh UNIQUE agar seed idempotent (satu item per menu)
ALTER TABLE `coming_soon_items`
  ADD UNIQUE KEY `uq_comingsoon_menu` (`nav_menu_id`);

-- Fix 7: credit_packages butuh UNIQUE agar seed idempotent
ALTER TABLE `credit_packages`
  ADD UNIQUE KEY `uq_package_name` (`name`);

-- Jaring pengaman tambahan untuk saldo kredit (MariaDB 10.2+ / MySQL 8.0.16+)
ALTER TABLE `credit_wallets`
