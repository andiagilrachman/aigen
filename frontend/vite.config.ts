...
export default defineConfig(({ mode }) => {
  /**
   * Vite tidak memuat berkas .env ke process.env saat mengevaluasi berkas ini —
   * .env hanya sampai ke import.meta.env di kode klien. Jadi membaca
   * process.env.VITE_BACKEND_ORIGIN di sini akan selalu undefined dan proxy
   * diam-diam jatuh ke nilai bawaan. loadEnv() yang membacanya dari .env.
   *
   * Di XAMPP biasanya http://localhost/aigen-backend/public
   */
  const env = loadEnv(mode, process.cwd(), '');
  const BACKEND_ORIGIN =
    env.VITE_BACKEND_ORIGIN || process.env.VITE_BACKEND_ORIGIN || 'http://127.0.0.1:8080';
